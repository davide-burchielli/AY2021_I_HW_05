...
#define SENSITIVITY_G 2  // Sensitivity in mg/digit. It is defined in the Datasheet since:
                         // - FS bits (CTRL_REG4) are set to 01
...
void SearchDRindex (uint8_t eeprom_value)  
    ...
    // SetRegister function is used at startup to setting CTRL_REG1 e CTRL_REG4
ErrorCode SetRegister (uint8_t RegisterAddress, uint8_t RegisterValue)
    {
        ...
        if (error == ERROR)  
            ...
            {
                if (reg_value != RegisterValue)  // If RegisterValue is not already equal to the value of the register
                {
    ...
    // UpdateCTRL_REG1 function update CTRL_REG1 with the new Data Rate
void UpdateCTRL_REG1(uint8_t DataRateToUpdate)
    {
        ...
        if (error == ERROR)  
            UART_Debug_PutString("ERROR : reading register in UpdateCTRL_REG1 function\r\n"); 
            {
                ...
                // with DataRateToUpdate to set 4 most significant bits.          
...
void ConvertAcc (uint8_t* AccelerationValues)
{
    k=0;
    j=0;
    while (k<6)  // Repeate the cycle for until the 3 axis accelerations are saved into DataBuffer array.
    {
        ...
        // 
        DataBuffer[k+1] = (uint8_t)(AccValuesConverted[j]& 0xFF);  // Save the MSB of AccValuesConverted[j] as the (k+1)° element of DataBuffer array
        DataBuffer[k+2] = (uint8_t)(AccValuesConverted[j] >> 8);   // Save the LSB of AccValuesConverted[j] as the (k+2)° element of DataBuffer array
    ...
    // UpdateEEPROM function write DataRateArray[DRindex] on the EEPROM_STARTUP_REGISTER
void UpdateEEPROM()
{                      
